...
describe('Budget Page - Merchant Removal', () => {
	...
	it('should expose the infinite loop by testing the View All Merchants button', async () => {
		...
		// Wait for the initial API call to complete first
		await waitFor(() => {
			expect(mockFetch).toHaveBeenCalledTimes(1);
		});

		// Wait a bit for the component to finish loading and updating its state
		await new Promise((resolve) => setTimeout(resolve, 100));

		// Wait for the merchant picker to load and render the combobox
	...
	it.skip('should validate the fix with exact production scenario: UI updates correctly after removal', async () => {
		...
		if (merchantStillVisible) {
			...
			console.log(
				'🚨 PRODUCTION BUG CONFIRMED: Second removal attempt fails because merchant already gone from data'
			);
		} else {
	...
	it.skip('should reproduce the exact production bug: UI never updates after removal', async () => {
		...
		await fireEvent.change(selectElement, {
			target: { value: 'CURSOR, AI POWERED IDE CURSOR.COM' }
		});
		...
		// Check if the UI updated after first removal
		const merchantListAfterFirstRemove = container.querySelector('.merchant-list');
		const merchantStillVisible = merchantListAfterFirstRemove.textContent.includes(
