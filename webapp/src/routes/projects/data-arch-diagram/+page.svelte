<script>
	import DataArchitectureHTML from './DataArchitectureHTML.svelte';
	import Header from '$lib/components/Header.svelte';
	import Footer from '$lib/components/Footer.svelte';
</script>

<svelte:head>
	<title>Gemini 3 Data Architecture Diagram</title>
</svelte:head>

<div class="min-h-screen bg-zinc-900 flex flex-col">
	<Header />
	<main class="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 relative">
		<!-- New Header Section -->
		<h1 class="text-3xl font-bold text-white text-center my-8">
			Gemini 3: From Sketch to Interactive Diagram
		</h1>
		<div class="max-w-4xl mx-auto mb-12 text-center text-gray-300 space-y-6">
			<p class="text-lg">
				I tested the impressive <strong>multi-modal capabilities of Gemini 3</strong> by uploading a
				rough hand-drawn sketch of a data architecture and asking it to generate a fully functional,
				interactive HTML/Svelte diagram.
			</p>
			<p>
				The result below highlights how Gemini 3 can interpret visual spatial relationships and text,
				translating them into semantic structure and styled components with minimal prompting.
			</p>
		</div>

		<!-- Comparison Section -->
		<div class="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
			<!-- Original Image -->
			<div class="flex flex-col gap-4">
				<div class="bg-zinc-800/50 rounded-xl p-6 border border-zinc-700 shadow-xl">
					<h2 class="text-xl font-bold text-emerald-400 mb-4 flex items-center gap-2">
						<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"
							><path
								stroke-linecap="round"
								stroke-linejoin="round"
								stroke-width="2"
								d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"
							></path></svg
						>
						The Input: Original Sketch
					</h2>
					<div class="relative rounded-lg overflow-hidden border border-zinc-600 bg-black">
						<img
							src="/images/original-architecture-sketch.jpeg"
							alt="Original hand-drawn data architecture sketch"
							class="w-full h-auto object-contain max-h-[800px]"
						/>
					</div>
					<p class="text-sm text-gray-400 mt-4 italic">
						A quick whiteboard-style sketch provided as the only reference material.
					</p>
				</div>
			</div>

			<!-- Generated Diagram -->
			<div class="flex flex-col gap-4">
				<div
					class="bg-zinc-800/50 rounded-xl p-1 border border-zinc-700 shadow-xl overflow-hidden"
				>
					<div class="p-5 pb-0">
						<h2 class="text-xl font-bold text-emerald-400 mb-4 flex items-center gap-2">
							<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"
								><path
									stroke-linecap="round"
									stroke-linejoin="round"
									stroke-width="2"
									d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4"></path></svg
							>
							The Output: Generated Code
						</h2>
					</div>
					<!-- Wrapper to ensure diagram containment -->
					<div class="diagram-container w-full overflow-x-auto pb-4 px-2">
						<div class="min-w-[350px]">
							<DataArchitectureHTML />
						</div>
					</div>
					<p class="text-sm text-gray-400 px-6 pb-6 italic">
						Interactive Svelte component generated by Gemini 3. Click items to explore.
					</p>
				</div>
			</div>
		</div>
	</main>
	<Footer />
</div>
