...
<svelte:head>
	...
	<meta name="description" content="Control and inspect stateful browser automation agent swarms in real-time." />
</svelte:head>
	...
	<div class="max-w-6xl w-full mx-auto px-4 py-12 flex-grow space-y-8">
		...
		<div class="text-center space-y-4">
			...
			<p class="text-slate-400 max-w-2xl mx-auto text-sm md:text-base">
				Generate short-lived HMAC signatures, instantiate Durable Object agent clients, and orchestrate browser-rendering Puppeteer sessions in real-time.
			</p>
		</div>
		...
		<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
			...
			<div class="bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 space-y-6 shadow-xl">
				...
				<!-- Worker Host Input -->
				<div class="space-y-2">
					<label for="workerHost" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Worker WebSocket Host</label>
					<input
						type="text"
						id="workerHost"
				...
				<div class="space-y-3">
					...
					<div class="flex flex-col gap-2">
						{#each personas as p}
							<button
								type="button"
								onclick={() => (persona = p)}
								class="text-left text-xs px-3.5 py-3 rounded-xl border transition-all duration-200 {persona === p ? 'bg-indigo-600/15 border-indigo-500/80 text-white font-medium shadow-md shadow-indigo-600/5' : 'bg-slate-950/40 border-slate-800/80 text-slate-400 hover:bg-slate-800/50'}"
							>
								{p}
							</button>
						{/each}
					...
					<div class="space-y-1.5">
						<span class="text-[10px] text-slate-500 uppercase tracking-wider">Or enter custom persona</span>
						<input
							type="text"
							bind:value={persona}
				...
				<div class="pt-4 border-t border-slate-800/60 space-y-4">
					...
					{#if errorMsg}
						<div class="bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-xl p-3.5 text-xs">
							<strong>Swarm Error:</strong> {errorMsg}
						</div>
					{/if}
				</div>
			</div>
					...
					{#if limits}
						<div class="bg-slate-950/50 border border-slate-800/60 rounded-xl p-4 space-y-3">
							...
							<div class="flex justify-between items-center text-xs border-t border-slate-800/60 pt-3">
								...
								<span class="text-slate-200 font-mono">{formatSeconds(limits.browser.timeUntilNextAllowedBrowserAcquisition || limits.browser.timeUntilNextAcquisition)}</span>
							</div>
							{/if}
							<div class="flex justify-between items-center text-xs border-t border-slate-800/60 pt-3">
			...
			<div class="bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-6 lg:col-span-2 flex flex-col h-[520px] shadow-xl">
				<div class="border-b border-slate-800 pb-3 flex items-center justify-between">
					<h2 class="text-lg font-bold text-white flex items-center gap-2">
						<svg class="w-5 h-5 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				...
				>
					{#if logs.length === 0}
						<div class="text-slate-500 flex flex-col items-center justify-center h-full text-center space-y-2">
							<svg class="w-8 h-8 text-slate-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
								<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
							</svg>
							<span>Terminal is currently idle.<br>Select a persona and engage the swarm to start.</span>
						</div>
					{/if}
