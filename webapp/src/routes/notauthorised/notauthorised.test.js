...
describe('NotAuthorised Page', () => {
	...
	it('renders the main title with glitch effect', () => {
		...
		expect(titles[0].className).toContain('text-green-400');
	});
	...
	it('has correct meta tags', () => {
		...
		expect(meta.getAttribute('content')).toBe('Authentication required - Some tools require login while under development');
	});
	...
	it('has proper color scheme', () => {
		...
		// Check green accent color for main title
		const titles = screen.getAllByText('ACCESS DENIED');
		expect(titles[0].className).toContain('text-green-400');
	...
	it('has proper section structure', () => {
		...
		const greenBorder = document.querySelector('.border-green-400\\/30');
