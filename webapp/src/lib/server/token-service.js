...
export class TokenService {
     ...
     */
    async storeToken(userId, serviceName, accessToken, refreshToken = null, expiresAt = null, refreshTokenExpiresAt = null) {
        try {
            const encryptedAccessToken = await encrypt(accessToken);
            const encryptedRefreshToken = refreshToken ? await encrypt(refreshToken) : null;

            const { success } = await this.d1.prepare(
            ...
            if (success) {
                ...
                logError(`Failed to store/update token for user ${userId}, service ${serviceName}`, 'TOKEN');
            }
            return success;
        } catch (error) {
            logError(`Error storing token for user ${userId}, service ${serviceName}: ${error.message}`, 'TOKEN', error);
            return false;
        }
    }
    ...
    async getTokensByUserId(userId) {
        try {
            const { results } = await this.d1.prepare(
                `SELECT * FROM UserStoredAuthToken WHERE userId = ? AND isRevoked = FALSE;`
            ).bind(userId).all();

            const decryptedTokens = await Promise.all(results.map(async (row) => {
                const decryptedAccessToken = await decrypt(row.encryptedToken);
                const decryptedRefreshToken = row.encryptedRefreshToken ? await decrypt(row.encryptedRefreshToken) : null;
                return {
                    ...row,
    ...
    async getToken(userId, serviceName) {
        try {
            ...
            logError(`Error retrieving token for user ${userId}, service ${serviceName}: ${error.message}`, 'TOKEN', error);
            return null;
        }
    }
    ...
    async revokeUserToken(userId, serviceName) {
        try {
            ...
            ).bind(new Date().toISOString(), userId, serviceName).run();
            ...
            logError(`Error revoking token for user ${userId}, service ${serviceName}: ${error.message}`, 'TOKEN', error);
            return false;
        }
    }
}
