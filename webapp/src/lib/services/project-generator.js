...
This project was generated with the following capabilities: ${projectConfig.selectedCapabilities.join(', ')}`
	...
	async generateProject(projectConfig, platform, request, explicitGitHubToken = null) {
		...
		let owner = projectConfig.repositoryUrl ? projectConfig.repositoryUrl.split('/')[3] : undefined;
		...
		// Update owner from created repository if it was undefined
		if (!owner && githubResult.repository?.fullName) {
			owner = githubResult.repository.fullName.split('/')[0];
		}
	...
	async setupGitHubRepository(githubApiService, owner, repoName, projectConfig) {
		let repository;
		try {
			log(`Creating GitHub repository: ${owner || 'new'}/${repoName}`, 'GEN');
			repository = await githubApiService.createRepository(
				repoName,
				false,
				`Project generated by genproj: ${projectConfig.projectName}`
			);
			// Update owner if not provided (e.g. when creating new repo without specified URL)
			if (!owner && repository.fullName) {
		...
		return { success: true, files: preview.files, repository };
	}
