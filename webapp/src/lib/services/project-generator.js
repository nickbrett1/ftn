...
export class ProjectGeneratorService {
    ...
    async generatePreview(projectConfig) {
        ...
        log('Generating project preview...', 'GEN', { projectName: projectConfig.projectName, capabilities: projectConfig.selectedCapabilities });
...
This project was generated with the following capabilities: ${projectConfig.selectedCapabilities.join(', ')}`,
    ...
    async generateProject(projectConfig, platform, request) {
        log('Generating full project...', 'GEN', { projectName: projectConfig.projectName, capabilities: projectConfig.selectedCapabilities });
        ...
        if (!githubToken) {
            return { success: false, message: 'GitHub token not found. Please authenticate with GitHub.' };
        }
        ...
        try {
            ...
            await githubApiService.createRepository(repoName, false, `Project generated by genproj: ${projectConfig.projectName}`);
        ...
        try {
            ...
            await githubApiService.createMultipleFiles(owner, repoName, preview.files, `Initial commit for ${projectConfig.projectName}`);
            ...
            return { success: false, message: `Failed to commit files to GitHub repository: ${error.message}` };
        }
                    ...
                    case 'circleci': {
                        ...
                        if (circleciToken) {
                            ...
                            try {
                                ...
                                externalServiceResults.push({ service: 'CircleCI', success: true, message: 'CircleCI configured successfully' });
                                log('CircleCI configured successfully', 'GEN');
                            } catch (error) {
                                logError(`CircleCI configuration failed: ${error.message}`, 'GEN', error);
                                externalServiceResults.push({ service: 'CircleCI', success: false, message: `CircleCI configuration failed: ${error.message}` });
                            }
                        } else {
                            externalServiceResults.push({ service: 'CircleCI', success: false, message: 'CircleCI token not found.' });
                        }
                        break;
                    }
                    case 'doppler': {
                        ...
                        if (dopplerToken) {
                            ...
                            try {
                                await dopplerApiService.createProject(projectConfig.projectName, `Secrets for ${projectConfig.projectName}`);
                                externalServiceResults.push({ service: 'Doppler', success: true, message: 'Doppler configured successfully' });
                                log('Doppler configured successfully', 'GEN');
                            } catch (error) {
                                logError(`Doppler configuration failed: ${error.message}`, 'GEN', error);
                                externalServiceResults.push({ service: 'Doppler', success: false, message: `Doppler configuration failed: ${error.message}` });
                            }
                        } else {
                            externalServiceResults.push({ service: 'Doppler', success: false, message: 'Doppler token not found.' });
                        }
                        break;
                    }
                    case 'sonarcloud': {
                        ...
                        if (sonarcloudToken) {
                            ...
                            try {
                                ...
                                await sonarcloudApiService.createProject(organization, projectConfig.projectName, projectConfig.projectName);
                                externalServiceResults.push({ service: 'SonarCloud', success: true, message: 'SonarCloud configured successfully' });
                                log('SonarCloud configured successfully', 'GEN');
                            } catch (error) {
                                logError(`SonarCloud configuration failed: ${error.message}`, 'GEN', error);
                                externalServiceResults.push({ service: 'SonarCloud', success: false, message: `SonarCloud configuration failed: ${error.message}` });
                            }
                        } else {
                            externalServiceResults.push({ service: 'SonarCloud', success: false, message: 'SonarCloud token not found.' });
                        }
                        break;
                    }
        ...
        log(`Configured ${externalServiceResults.filter(r => r.success).length} external services`, 'GEN');

        return { success: true, message: 'Project generation completed successfully.', files: preview.files, externalServiceResults };
    }
}