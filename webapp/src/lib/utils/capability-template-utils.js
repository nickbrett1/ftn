...
function _applyDockerContainerConfig(data, context, contextEnabled, contextName) {
	...
	const armBuilds = config.armBuilds === true;
	const buildPlatforms = armBuilds ? 'linux/amd64,linux/arm64' : 'linux/amd64';
	// GHCR package visibility. Defaults to `public` (no NAS-side credentials
	// for Watchtower pulls); opt into `private` via config. The generated
	// Dockerfile's `org.opencontainers.image.source` label links the package to
	// the repo on first push, so a public repo yields a public package.
	const imageVisibility = config.imageVisibility || 'public';
