      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {#each categoryCapabilities as capability}
          <div 
            ...
            class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow {selectedCapabilities.includes(capability.id) ? 'ring-2 ring-blue-500 bg-blue-50' : 'bg-white'}"
          >
            <!-- Capability Header -->
                        ...
                        <div class="text-sm">
                          {#if rules.enum}
                            <select 
                              ...
                              on:change={(e) => handleConfigurationChange(capability.id, field, e.target.value)}
                            >
                          ...
                          {:else if rules.type === 'boolean'}
                            <input 
                              ...
                              on:change={(e) => handleConfigurationChange(capability.id, field, e.target.checked)}
                            />
                          {:else if rules.type === 'array'}
                            <input 
                              ...
                              on:change={(e) => handleConfigurationChange(capability.id, field, e.target.value.split(',').map(s => s.trim()).filter(s => s))}
                            />
                          {:else}
                            <input 
                              type={rules.type || 'text'} 
...
{#if selectedCapabilities.some(id => capabilities.find(c => c.id === id)?.dependencies?.length > 0)}
  <div data-testid="dependency-warning" class="bg-yellow-50 border border-yellow-200 rounded-md p-4 mt-6">
    <div class="flex">
...
<!-- Conflict Warnings -->
{#if selectedCapabilities.some(id => capabilities.find(c => c.id === id)?.conflicts?.some(conflict => selectedCapabilities.includes(conflict)))}
...
{#if selectedCapabilities.some(id => capabilities.find(c => c.id === id)?.requiresAuth?.length > 0)}
  <div data-testid="auth-requirements" class="bg-blue-50 border border-blue-200 rounded-md p-4 mt-6">
    <div class="flex">
      <div class="flex-shrink-0">
