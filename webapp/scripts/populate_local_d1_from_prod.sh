...
show_usage() {
    ...
    echo ""
    echo "Arguments:"
    echo "  DATABASE_NAME    Database to populate from production (wdi, ccbilling, or api-keys)"
...
if [ "$DB_NAME" != "wdi" ] && [ "$DB_NAME" != "ccbilling" ] && [ "$DB_NAME" != "api-keys" ]; then
...
if [ "$NO_TABLES_FOUND" = false ]; then
    ...
    exit 2 # Skipped, exit with code 2
fi
