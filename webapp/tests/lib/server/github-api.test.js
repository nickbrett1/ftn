...
describe('GitHubAPIService', () => {
	...
	it('creates or updates a repository secret', async () => {
		...
		const keyJson = vi.fn().mockResolvedValue({ key: '/qKPNvqjqqVSQ6HlREVr10yqNv6g09wcUdSBthIYBTs=', key_id: '123' });

		vi.spyOn(service, 'makeRequest')
			.mockResolvedValueOnce({ json: keyJson })
			.mockResolvedValueOnce({});

		await service.createRepositorySecret('user', 'repo', 'MYTOKEN', 'mysecretvalue');

		// Check if it got the public key
		expect(service.makeRequest).toHaveBeenCalledWith('/repos/user/repo/actions/secrets/public-key');

		// Check if it set the secret
		const [, putOptions] = service.makeRequest.mock.calls.at(-1);
		expect(service.makeRequest).toHaveBeenLastCalledWith('/repos/user/repo/actions/secrets/MYTOKEN', expect.anything());
