...
vi.mock('../../../src/lib/config/capabilities.js', () => ({
	capabilities: [
        { id: 'devcontainer-node', name: 'Node.js DevContainer', templates: [
            ...
            { templateId: 'devcontainer-post-create-setup-sh', filePath: '.devcontainer/post-create-setup.sh' },
        ]},
        { id: 'devcontainer-java', name: 'Java DevContainer', templates: [
		...
		{ id: 'another', name: 'Another Capability', templates: [{templateId: 'another-template', filePath: 'another/file.txt'}] }
	]
}));

vi.mock('../../../src/lib/utils/capability-resolver.js', () => ({
	resolveDependencies: vi.fn((caps) => ({ resolvedCapabilities: caps, addedDependencies: [], conflicts: [], isValid: true })),
...
describe('generatePreview', () => {
    ...
    it('merges multiple devcontainer files correctly', async () => {
        ...
        const preview = await generatePreview(projectConfig, ['devcontainer-node', 'devcontainer-java']);
        
        const devcontainerFolder = preview.files.find(f => f.name === '.devcontainer' && f.type === 'folder');
        expect(devcontainerFolder).toBeDefined();

        const devcontainerJsonFile = devcontainerFolder.children.find(f => f.name === 'devcontainer.json');
