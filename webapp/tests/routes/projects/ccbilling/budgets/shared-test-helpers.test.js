...
describe('test CRUD wrapper', () => {
	...
	it('testBudgetCRUD create works', async () => {
		await helpers.testBudgetCRUD.create(InteractiveMockComponent, {
			buttonText: 'Add New Budget',
	...
	it('testBudgetCRUD update works', async () => {
		await helpers.testBudgetCRUD.update(MockEditComponent, {
			data: { budgets: [{ id: 1 }] },
	...
	it('testBudgetCRUD delete works', async () => {
		await helpers.testBudgetCRUD.delete(MockDeleteComponent, {
			data: { budgets: [{ id: 1 }] },
			expectedUrl: '/projects/ccbilling/budgets/1'
		});
	});
	...
	it('testMerchantCRUD create works', async () => {
		await helpers.testMerchantCRUD.create(InteractiveMockComponent, {
	...
	it('testMerchantCRUD delete works', async () => {
		await helpers.testMerchantCRUD.delete(MockDeleteComponent, {
			buttonText: 'Remove',
			expectedUrl: '/projects/ccbilling/budgets/1/merchants',
	...
	it('testLoadingState works correctly', async () => {
		const { resolvePromise } = await helpers.testLoadingState(
			InteractiveMockComponent,
			{ text: 'val' },
			{
				buttonText: 'Add New Budget',
	...
	it('testFormValidation works correctly', async () => {
		await helpers.testFormValidation(
			InteractiveMockComponent,
			{ text: 'val' },
			'Add New Budget',
			'error'
	...
	it('createDetailData returns object', () => {
		expect(helpers.createDetailData({ id: 1 }, [{ merchant: 'W' }])).toEqual({
			budget: { id: 1 },
			merchants: [{ merchant: 'W' }]
		});
	});
