...
test.describe('Progressive Authentication Flow Integration', () => {
	test('should prompt for GitHub login when a GitHub-dependent capability is selected by an unauthenticated user', async ({
		page
	}) => {
	...
	test('should not prompt for login if no auth-dependent capabilities are selected', async ({
		page
	}) => {
		await page.goto('/projects/genproj');
	...
	test('should show a success message after successful authentication and return to genproj page', async ({
		page
	}) => {
