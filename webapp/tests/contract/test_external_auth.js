...
test.describe('External Service Auth Endpoints Contract', () => {
	...
	for (const serviceName of services) {
		test(`GET /api/projects/genproj/api/auth/${serviceName} should redirect to ${serviceName} OAuth`, async ({
			request
		}) => {
		...
		test(`GET /api/projects/genproj/api/auth/${serviceName}/callback should handle OAuth callback`, async ({
			request
		}) => {
			const dummyCode = `dummy_${serviceName}_code`;
			const response = await request.get(
				`/api/projects/genproj/api/auth/${serviceName}/callback?code=${dummyCode}`,
				{
					maxRedirects: 0 // Do not follow redirects
				}
			);
		...
		test(`GET /api/projects/genproj/api/auth/${serviceName}/callback should handle OAuth errors`, async ({
			request
		}) => {
			const errorResponse = await request.get(
				`/api/projects/genproj/api/auth/${serviceName}/callback?error=access_denied`,
				{
					maxRedirects: 0 // Do not follow redirects
				}
			);
