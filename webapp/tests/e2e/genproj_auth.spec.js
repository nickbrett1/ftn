...
test.describe('Genproj Authentication Workflow E2E', () => {
	test('should redirect to GitHub auth when attempting to generate without authentication', async ({
		page
	}) => {
	...
	test('should show auth flow when authenticated user tries to generate', async ({ page }) => {
		...
		await page
			.context()
			.addCookies([{ name: 'auth', value: 'mock-auth-token', url: 'http://localhost:5173' }]);
		await page.goto('/projects/genproj');
